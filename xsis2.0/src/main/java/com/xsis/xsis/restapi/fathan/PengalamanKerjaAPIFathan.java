...
import com.xsis.xsis.dto.fathan.PengalamanKerjaDto;
...
@RestController
@RequestMapping("/api/pengalaman-kerja-fathan")
@CrossOrigin(origins = "*")
public class PengalamanKerjaAPIFathan {

    @Autowired
    private PengalamanKerjaServiceFathan pengalamanKerjaService;

    @GetMapping
    public List<PengalamanKerjaFathan> getId() {
        return pengalamanKerjaService.getAll();
    }

    @GetMapping("/{id}")
    public PengalamanKerjaFathan getPengalamanId(@PathVariable(value = "id") Long id) {
        return pengalamanKerjaService.getById(id);
    }

    @PostMapping
    public PengalamanKerjaDto simpan(@RequestBody PengalamanKerjaDto kerjaDto) {
        PengalamanKerjaFathan pengalaman = new PengalamanKerjaFathan();
